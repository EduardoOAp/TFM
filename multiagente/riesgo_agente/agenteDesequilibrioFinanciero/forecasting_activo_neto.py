...
from riesgo_agente.utils.config import AGENTE_DESEQUILIBRIO, DES_MODULO_FORECASTING
...
class ForecastingActivoNeto:
    def __init__(self, df: pd.DataFrame, columna_objetivo: str, columnas_exogenas: List[str], horizonte: int = 6):
        ...
        self.modulo = DES_MODULO_FORECASTING
        self.resultado = []
        self.fechaTexto = pd.to_datetime(df['fecha'], errors='coerce').max().strftime('%Y%m%d')
        self.ruta_resultado = os.path.join(DATA_ROOT, RESULTADOS_ROOT, AGENTE_DESEQUILIBRIO, DES_MODULO_FORECASTING, self.fechaTexto)
        self.ruta_modelo = os.path.join(DATA_ROOT, MODELOS_ROOT, AGENTE_DESEQUILIBRIO, DES_MODULO_FORECASTING)
        self.ruta_temporal = os.path.join(DATA_ROOT, TEMPORAL_ROOT, AGENTE_DESEQUILIBRIO, DES_MODULO_FORECASTING)
    ...
    def guardar_modelos(self, ruta_base: str, entidad: str, fondo: str):
        ruta_modelo = os.path.join(ruta_base, self.agente, self.modulo)
        os.makedirs(ruta_modelo, exist_ok=True)
        #with open(f"{ruta_modelo}/{self.fondo}_modelo.pkl", "wb") as f:        
        if self.modelo_holt:
            with open(f"{ruta_modelo}/{fondo}_{entidad}_modelo.pkl", "wb") as f:
    ...
    def entrenar_por_entidad(self, df: pd.DataFrame, campo_entidad: str = "entidad", fondo: str = "ROP", ruta_modelos: str = "data/modelos") -> Dict[str, Any]:
        ...
        for entidad in entidades:
            ...
            try:
                ...
                self.guardar_modelos(ruta_modelos, entidad, fondo)
...
"""
def completar_fechas_mensuales(df, columna_fecha="fecha", columna_entidad="entidad"):
    df = df.copy()
...
if __name__ == "__main__":
    ...
    modulo = "Forecasting de liquidez activo neto"
    fecha = "2024-12-31"
    ...
    df_completo = completar_fechas_mensuales(df)    
    # Rellenar hacia adelante
    df_completo = df_completo.sort_values(["entidad", "fecha"])
    df_completo = df_completo.groupby("entidad", group_keys=False).apply(lambda g: g.ffill())
    df_completo = df_completo.reset_index(drop=True) 
    ...
    columnas_usar = df.columns.difference(["entidad", "fondo", "fecha","monto_activo_neto"]).tolist()

    modelo = ForecastingLiquidez(
        columna_objetivo=columna_objetivo,
        columnas_exogenas=columnas_usar, agente=agente, modulo=modulo
    )
    resultados = modelo.entrenar_por_entidad(df)
    graficar_forecasts(df, resultados, columna_objetivo)
    print(resultados)
